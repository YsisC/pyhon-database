...
class Bookdb:
    ...
    def save_results_to_file(self, results, filename='results_replicate.json'):
        with open(filename, 'w') as archivo:
            json.dump(results, archivo)

    def create_database_table(self):
        self.cursor.execute("""
            CREATE TABLE IF NOT EXISTS results_replicate (
    ...
    def view(self):
        self.cursor.execute("SELECT * FROM results_replicate")
    ...
    def insert(self,list):
        for item in list:
            self.cursor.execute("INSERT INTO results_replicate (data) VALUES (%s)", (json.dumps(item),))
